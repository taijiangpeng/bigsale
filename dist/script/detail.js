    ...
    success: function success(data) {
      ...
      data.forEach(function (ele, index) {
        ...
        if (ele['src'] == send_src) {
          // console.log(ele);
          if (this_index == 0) {
            $('.select_pd').text(ele['sprice']); // console.log(ele['sprice'].slice(4));
...
$.ajax({
  ...
  success: function success(data) {
    data.forEach(function (ele, index) {
      console.log(send_src, ele['src']); // console.log(ele,index);
...
$('.add').click(function () {
  ...
  $('.reduce').removeAttr('disabled'); // console.log(num,price_one);
