...
function animate(dom, attr_obj, callback) {
  ...
  dom.timer = setInterval(function () {
    for (var attr in attr_obj) {
      ...
      var target = attr_obj[attr].target; // 不断变化的速度

      var speed = (target - current) / 10; // 小数计算有误差，容易造成数据丢失 => 取整

      speed = speed > 0 ? Math.ceil(speed) : Math.floor(speed); // 判断到达目的地：剩余运动量 <= 每次的运动量
      ...
      if (Math.abs(target - current) <= Math.abs(speed)) {
        ...
        delete attr_obj[attr]; // 如果对象中还有其他属性，此时不应该终止计时器
...
function getQueryString(key) {
  ...
  var arr = searchStr.match(reg); // console.log(arr[1]);
...
function ajax(options) {
  ...
  if (isObject(options.data)) {
    ...
    data = data.substring(0, data.length - 1); // 'abc=123&www=baidu'
  }
  ...
  if (options.type.toLowerCase() === 'get') {
    ...
    xhr.open(options.type, options.url); // POST请求需要在send之前设置请求头，模拟表单的post提交
...
function jsonp(options) {
  ...
  window[options.jsonpCallback] = options.success; // 格式化参数
  ...
  var oScript = document.createElement('script'); // 把数据地址、参数、回调函数拼接赋值给src

  oScript.src = options.url + '?' + data + '&' + options.jsonp + '=' + options.jsonpCallback; // 添加到body中

  document.body.appendChild(oScript); // 数据加载完成后删除script标签
